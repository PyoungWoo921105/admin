...
export const GetMedicineListExport = async (data: any) => {
  const { CommonData } = useStore();
  try {
    /* axios.get(url[, config]) */
    const response = await customAxios.get('/medicine/list/export', {
      params: data,
      responseType: 'blob',
    });
    ...
    link.setAttribute(
      'download',
      `kpi_medicine raw data_${CommonData.CurrentTime.substring(0, 8)}.csv`
    );
...
export default GetMedicineListExport;
