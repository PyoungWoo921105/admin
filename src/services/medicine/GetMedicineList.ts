...
export const GetMedicineList = async (data: any) => {
  const { MedicineData } = useStore();
  try {
    /* axios.get(url[, config]) */
    const response = await customAxios.get('/medicine/list', { params: data });
    const MetaResponse = response as { status: number; data: any };
    MedicineData.setMedicineListData(MetaResponse.data);
    return MetaResponse;
  } catch (error: unknown) {
