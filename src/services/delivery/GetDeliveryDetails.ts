...
export const GetDeliveryDetails = async (data: any) => {
  const { DeliveryData } = useStore();
  try {
    /* axios.get(url[, config]) */
    const response = await customAxios.get('/delivery/details', { params: data });
    const MetaResponse = response as { status: number; data: any };
    DeliveryData.setDeliveryDetailsData(MetaResponse.data);
    return MetaResponse;
  } catch (error: unknown) {
