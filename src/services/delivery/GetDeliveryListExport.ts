...
export const GetDeliveryListExport = async (data: any) => {
  const { CommonData } = useStore();
  try {
    /* axios.get(url[, config]) */
    const response = await customAxios.get('/delivery/list/export', {
      params: data,
      responseType: 'blob',
    });
    ...
    link.setAttribute(
      'download',
      `kpi_delivery raw data_${CommonData.CurrentTime.substring(0, 8)}.csv`
    );
...
export default GetDeliveryListExport;
