...
export const GetTreatmentListExport = async (data: any) => {
  const { CommonData } = useStore();
  try {
    /* axios.get(url[, config]) */
    const response = await customAxios.get('/treat/list/export', {
      params: data,
      responseType: 'blob',
    });
    ...
    link.setAttribute(
      'download',
      `kpi_treatment raw data_${CommonData.CurrentTime.substring(0, 8)}.csv`
    );
...
export default GetTreatmentListExport;
