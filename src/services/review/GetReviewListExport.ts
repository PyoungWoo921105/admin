...
export const GetReviewListExport = async (data: any) => {
  const { CommonData } = useStore();
  try {
    /* axios.get(url[, config]) */
    const response = await customAxios.get('/review/list/export', {
      params: data,
      responseType: 'blob',
    });
    ...
    link.setAttribute(
      'download',
      `kpi_review raw data_${CommonData.CurrentTime.substring(0, 8)}.csv`
    );
...
export default GetReviewListExport;
