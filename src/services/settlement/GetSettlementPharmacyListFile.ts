...
export const GetSettlementPharmacyListFile = async (data: any) => {
  const { CommonData } = useStore();
  try {
    /* axios.get(url[, config]) */
    const response = await customAxios.get('/settlement/pharmacy/list/file', {
      params: data,
      responseType: 'blob',
    });
    ...
    link.setAttribute(
      'download',
      `kpi_pharmacy_settlement_${CommonData.CurrentTime.substring(0, 8)}.zip`
    );
...
export default GetSettlementPharmacyListFile;
