...
export const GetSettlementHospitalListFile = async (data: any) => {
  const { CommonData } = useStore();
  try {
    /* axios.get(url[, config]) */
    const response = await customAxios.get('/settlement/hospital/list/file', {
      params: data,
      responseType: 'blob',
    });
    ...
    link.setAttribute(
      'download',
      `kpi_hospital_settlement_${CommonData.CurrentTime.substring(0, 8)}.zip`
    );
...
export default GetSettlementHospitalListFile;
