...
export const GetPromotionListExport = async (data: any) => {
  const { CommonData } = useStore();
  try {
    /* axios.get(url[, config]) */
    const response = await customAxios.get('/promotion/list/export', {
      params: data,
      responseType: 'blob',
    });
    ...
    link.setAttribute(
      'download',
      `kpi_promotion raw data_${CommonData.CurrentTime.substring(0, 8)}.csv`
    );
...
export default GetPromotionListExport;
