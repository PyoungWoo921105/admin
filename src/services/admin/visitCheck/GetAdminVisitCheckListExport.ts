...
export const GetAdminVisitCheckListExport = async (data: any) => {
  const { CommonData } = useStore();
  try {
    /* axios.get(url[, config]) */
    const response = await customAxios.get('/admin/visitCheck/list/export', {
      params: data,
      responseType: 'blob',
    });
    ...
    link.setAttribute(
      'download',
      `kpi_visitCheck raw data_${CommonData.CurrentTime.substring(0, 8)}.csv`
    );
...
export default GetAdminVisitCheckListExport;
