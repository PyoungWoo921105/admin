...
export const GetAdminHospitalAdditionDetailsListExport = async (data: any) => {
  const { CommonData } = useStore();
  try {
    /* axios.get(url[, config]) */
    const response = await customAxios.get('/admin/hospitalAddition/details/list/export', {
      params: data,
      responseType: 'blob',
    });
    ...
    link.setAttribute(
      'download',
      `kpi_hospitalAddition raw data_${CommonData.CurrentTime.substring(0, 8)}.csv`
    );
...
export default GetAdminHospitalAdditionDetailsListExport;
